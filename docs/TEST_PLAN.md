...
6. **In progress.** MEDIUM/LOW component gaps, opportunistically. First batch landed: `PatientRow`, `PatientAvatar` (+ picker), `SignaturePad`-adjacent, `firebase/forms` + `firebase/formLinks`, and `AdminConsole` per-account actions (reset/delete/confirm) + section rendering. Moved components/app 57.9%→63.7%, components/admin 21.0%→24.4%, overall 53.4%→54.3% (1008 tests). Still open: the `app/app/patients` page shells, deeper `AdminConsole` flows (create-user, catalog/entity editors), and the thin `lib/firebase` live watchers — best picked up alongside feature work via TDD.
