...
Layout and content of the combined Treatment Authorisation PDF produced at approval — one
document per approved request covering all its items — for both the demo-mode web renderer
(`src/lib/demo/approvalPdf.ts`) and the LIVE backend Cloud Function renderer
(`backend/functions/src/authorisationPdf.ts` in the Aestheticx repo, the layout truth).
Reflects the 19/07 owner feedback that removed the Clause 68C boilerplate sections.
