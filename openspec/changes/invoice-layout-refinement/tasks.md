...
- [ ] 2.1 `addressLines()` helper + `buildTaxInvoiceModel` producing `sellerLines` and `toLines` arrays (name, trading name, ABN, address, email; missing lines omitted, ABN em-dash fallback); unit tests incl. legacy snapshot
