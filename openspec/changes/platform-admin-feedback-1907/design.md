...
2. Clinic ↔ doctor linking (change `clinic-account-access-and-linking`, web #136 + backend #110) always grants the doctor an **employee** clinic membership (`users/{uid}.clinics` map → claims → "Practise as" identity). The owner wants two kinds at creation: *employee* (works at the clinic) and *prescriber* (external doctor who authorises for the clinic, no membership) — reworked to a SET (`relationshipKinds`, ≥1 of the two) since a doctor can be both.
...
- `relationshipKinds: ('employee' | 'prescriber')[]` (non-empty set) on doctor↔clinic relationships, end-to-end (create form → demo reducer → callable → Firestore doc → hydration → row display), with membership sync keyed off `includes('employee')`.
- Absent kinds ⇒ `[employee]` everywhere (no migration; deploy-order safe); interim singular `relationshipKind` docs decode as a one-element set.
...
3. **Membership reconciliation reuses the existing grant/revoke paths.** `writeClinicMembership` already handles grant (status active) and provenance-aware revoke (status inactive). Kinds fold in as: grant iff `status === 'active' && kinds.includes('employee')`; otherwise run the revoke branch (which only touches memberships whose provenance is this relationship). Removing `employee` from an active relationship therefore behaves exactly like deactivation did for membership, and adding it like activation. `syncClaimsFromUserProfile` is unchanged and still runs for every clinic-counterparty call.
4. **Web types make the kinds optional (`relationshipKinds?: RelationshipKind[]`)** and treat `undefined`/garbled as `[employee]` for clinic rows at display/derivation sites via a tiny helper `effectiveRelationshipKinds(rel)` in `types.ts` (returns `null` for nurse counterparties, canonical order otherwise). Keeps nurse rows kind-free without a union explosion.
5. **Demo parity**: the demo reducer validates kinds (clinic-only, non-empty, valid values) and stores the deduped canonical set; `identity.ts` grants the demo employee clinic identity only when the effective set includes employee. Audit summary strings include the set for clinic relationships so history reads "created · employee+prescriber · active · …".
