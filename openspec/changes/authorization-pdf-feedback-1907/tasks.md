...
- [ ] 1.1 Update `src/lib/demo/__tests__/approval-pdf.test.ts` first (TDD red): new
...
- [ ] 1.2 Update `src/lib/demo/approvalPdf.ts`: change `DEFAULT_TIMING`; drop
...
- [ ] 1.3 Web tests green (`npx vitest run src/lib/demo/__tests__/approval-pdf.test.ts` then
      the full suite); commit.

## 2. Backend renderer (Aestheticx repo, coupled PR)

- [ ] 2.1 Create a feature branch in /Users/zhendeng/Documents/Aestheticx; update
...
- [ ] 2.2 Update `backend/functions/src/authorisationPdf.ts`: same removals; header contact
...
- [ ] 2.3 Backend tests + build green (`npm test` / `tsc`) in backend/functions; commit.

## 3. Review, verify, ship

- [ ] 3.1 Engineer review (code-reviewer) on both diffs; fix CRITICAL/HIGH; re-run tests.
- [ ] 3.2 Full web suite + lint/build green; full backend suite green.
- [ ] 3.3 Sync delta spec into main specs (`openspec-sync-specs`).
