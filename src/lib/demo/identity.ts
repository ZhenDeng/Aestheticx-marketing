...
import { effectiveRelationshipKind, type CooperationRelationship, type Identity } from "./types";
...
export function heldIdentities(
  ...
  // an active doctor↔clinic relationship of kind employee grants the doctor an employee
  // clinic identity. A prescriber relationship is an external cooperation only — it gates
  // authorisation requests but confers no membership.
  for (const relationship of demoRelationships) {
    if (relationship.status !== "active" || relationship.counterpartyType !== "clinic") continue;
    if (effectiveRelationshipKind(relationship) !== "employee") continue;
