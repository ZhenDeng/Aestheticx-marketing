...
describe("profileForUser", () => {
  it("defaults to empty fields when the user has no stored profile", () => {
    expect(profileForUser(emptyState(), "u-voss")).toEqual({ ahpra: "", abn: "", phone: "", address: "", principalPlace: "", premises: [] });
  });

  it("returns the stored profile when present", () => {
    const s = {
      ...emptyState(),
      profileByUser: { "u-voss": { ahpra: "MED0001", abn: "82 601 443 218", phone: "0412 884 209", address: "14 Acland St", principalPlace: "", premises: [] } },
    };
...
describe("updateProfile", () => {
  it("merges a partial edit onto the default when no prior profile exists", () => {
    ...
    expect(profileForUser(s, "u-voss")).toEqual({ ahpra: "", abn: "", phone: "0400 000 000", address: "", principalPlace: "", premises: [] });
  });
  ...
  it("merges an edit without disturbing other fields", () => {
    ...
    expect(profileForUser(s, "u-voss")).toEqual({
      ahpra: "MED0001", abn: "", phone: "0400 000 000", address: "14 Acland St", principalPlace: "", premises: [],
    });
  });
